    ...
    >
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 flex items-center gap-2 px-3 py-2">
            ...
            {product.units && product.units.length > 0 && (
              <div className="space-y-0.5">
                ...
                <div className="flex flex-col gap-0.5">
                  {product.units.map((u) => <span key={u} className="px-1 py-0.5 bg-blue-100 text-blue-700 rounded text-[8px] font-medium whitespace-nowrap">{u}</span>)}
                </div>
              </div>
            )}
            {product.sizes && product.sizes.length > 0 && (
              <div className="space-y-0.5">
                ...
                <div className="flex flex-col gap-0.5">
                  {product.sizes.map((s) => <span key={s} className="px-1 py-0.5 bg-green-100 text-green-700 rounded text-[8px] font-medium whitespace-nowrap">{s}</span>)}
                </div>
              </div>
            )}
          </div>
          ...
          {/* Center: Image */}
          <div className="relative flex-1 min-w-0 rounded-xl overflow-hidden bg-gray-50">
            <div className="aspect-square">
              ...
              />
              {activeDiscount && (
                <div className="absolute top-1 left-1 bg-red-500 text-white px-1.5 py-0.5 rounded-full text-[9px] font-bold" data-testid={`badge-discount-${product.id}`}>
                ...
                {product.colors.map((c) => {
                  ...
                  return (
                    <div key={colorName} className="flex items-center gap-1" title={colorName}>
                      <span 
                        style={{width: '14px', height: '14px', backgroundColor: colorHex, borderRadius: '2px', border: '1px solid rgba(0,0,0,0.15)'}}
